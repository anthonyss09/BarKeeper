...
## ready build and deploy via heroku
...
- in server.js
- for es6 modules import dirname, fileURLToPath and path...
- neccessary to serve static assets from build folder
...
app.use(express.static(path.resolve(\_\_dirname,'./client/buil')))

## direct all get routes that don't use auth and product routes to static build and html
...
- spin up localhost:5000 with node server command

## security packages & rate limit
...
- rm -rf .git (remove existing repositories)
- git init
- git add .
